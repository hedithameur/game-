...
{  int a;
	srand(time(NULL));//intialise le generateur de nombres aleatoires
 *n= 1+ rand()%6; //donner un nombre entre 0 et 6
  a=(*n);
 return a;
}
...
char question[100];
 ...
 sprintf(question ,"calculate squared of %d",n);//envoie la sortie formatee par une chaine pointee
...
void afficher_resultat (SDL_Surface * screen,int solution,int r)
 {SDL_Surface *background; background=NULL;
 	...
 	if (r==solution)
 	{
 		...
 		SDL_BlitSurface(background, NULL, screen, NULL) ;
        SDL_Flip(screen);
	SDL_Delay(3000);//permet de mettre en pause le programme un certain nombre de millisecondes
 	}
 	else
 	{
